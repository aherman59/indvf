...
--
-- Début d'exécution le 05/10/2016 à 10:17:02
---
...
COMMENT ON COLUMN dvf.local_plus.idpar IS 'identifiant de la parcelle (idem Fichiers fonciers)';

COMMENT ON COLUMN dvf.local_plus.coddep IS 'code du département';

COMMENT ON COLUMN dvf.local_plus.iddispoloc IS 'identifiant de clef primaire de la table local';
...
COMMENT ON COLUMN dvf.disposition_parcelle_plus.idpar IS 'identifiant de la parcelle (idem Fichiers fonciers)';
...
COMMENT ON COLUMN dvf.mutation_plus.nbmai1pp IS 'nombre de maisons avec au plus une pièce principale ayant muté';
...
COMMENT ON COLUMN dvf.mutation_plus.nbapt1pp IS 'nombre d''appartements avec au plus une pièce principale ayant muté';

COMMENT ON COLUMN dvf.mutation_plus.libnatmut IS 'libellé de la nature de mutation';

COMMENT ON COLUMN dvf.mutation_plus.sapt1pp IS 'surface de l''ensemble des appartements avec au plus une pièce principale ayant muté';

COMMENT ON COLUMN dvf.mutation_plus.refdoc IS 'référence d''enregistrement du document (acte de vente)';

COMMENT ON COLUMN dvf.mutation_plus.smai1pp IS 'surface de l''ensemble des maisons avec au plus une pièce principale ayant muté';
...
COMMENT ON COLUMN dvf_d62.local_plus.idpar IS 'identifiant de la parcelle (idem Fichiers fonciers)';

COMMENT ON COLUMN dvf_d62.local_plus.coddep IS 'code du département';

COMMENT ON COLUMN dvf_d62.local_plus.iddispoloc IS 'identifiant de clef primaire de la table local';
...
COMMENT ON COLUMN dvf_d62.disposition_parcelle_plus.idpar IS 'identifiant de la parcelle (idem Fichiers fonciers)';
...
COMMENT ON COLUMN dvf_d62.mutation_plus.nbmai1pp IS 'nombre de maisons avec au plus une pièce principale ayant muté';
...
COMMENT ON COLUMN dvf_d62.mutation_plus.nbapt1pp IS 'nombre d''appartements avec au plus une pièce principale ayant muté';

COMMENT ON COLUMN dvf_d62.mutation_plus.libnatmut IS 'libellé de la nature de mutation';

COMMENT ON COLUMN dvf_d62.mutation_plus.sapt1pp IS 'surface de l''ensemble des appartements avec au plus une pièce principale ayant muté';

COMMENT ON COLUMN dvf_d62.mutation_plus.refdoc IS 'référence d''enregistrement du document (acte de vente)';

COMMENT ON COLUMN dvf_d62.mutation_plus.smai1pp IS 'surface de l''ensemble des maisons avec au plus une pièce principale ayant muté';
...
COMMENT ON COLUMN dvf_d59.local_plus.idpar IS 'identifiant de la parcelle (idem Fichiers fonciers)';

COMMENT ON COLUMN dvf_d59.local_plus.coddep IS 'code du département';

COMMENT ON COLUMN dvf_d59.local_plus.iddispoloc IS 'identifiant de clef primaire de la table local';
...
COMMENT ON COLUMN dvf_d59.disposition_parcelle_plus.idpar IS 'identifiant de la parcelle (idem Fichiers fonciers)';
...
COMMENT ON COLUMN dvf_d59.mutation_plus.nbmai1pp IS 'nombre de maisons avec au plus une pièce principale ayant muté';
...
COMMENT ON COLUMN dvf_d59.mutation_plus.nbapt1pp IS 'nombre d''appartements avec au plus une pièce principale ayant muté';

COMMENT ON COLUMN dvf_d59.mutation_plus.libnatmut IS 'libellé de la nature de mutation';

COMMENT ON COLUMN dvf_d59.mutation_plus.sapt1pp IS 'surface de l''ensemble des appartements avec au plus une pièce principale ayant muté';

COMMENT ON COLUMN dvf_d59.mutation_plus.refdoc IS 'référence d''enregistrement du document (acte de vente)';

COMMENT ON COLUMN dvf_d59.mutation_plus.smai1pp IS 'surface de l''ensemble des maisons avec au plus une pièce principale ayant muté';
...
DROP TRIGGER IF EXISTS insert_local_plus_trigger ON dvf.local_plus;      
...
-- création du trigger de la table disposition_parcelle_plus
...
DROP TRIGGER IF EXISTS insert_disposition_parcelle_plus_trigger ON dvf.disposition_parcelle_plus;      
...
-- création du trigger de la table mutation_plus
...
DROP TRIGGER IF EXISTS insert_mutation_plus_trigger ON dvf.mutation_plus;      
...
CREATE TABLE dvf_d62.tmp_calcul_ann_nature_mutation_idmutation AS(         
    SELECT
        t1.idmutation, 
        t2.libnatmut
    FROM
...
CREATE TABLE  dvf_d62.tmp_calcul_disposition_parcelle_idmutation AS(
    ...
    FROM  dvf_d62.disposition_parcelle_plus t
    LEFT JOIN (SELECT idmutation, iddispopar, idpar FROM dvf_d62.disposition_parcelle_plus WHERE parcvendue = TRUE) tt
...
CREATE TABLE  dvf_d59.tmp_calcul_disposition_parcelle_idmutation AS(
    ...
    SELECT 
        ...
        -- A partir de la version 9.3 de PostgreSQL:
        -- array_remove(array_agg(DISTINCT tt.idpar), NULL) AS l_idparmut
    FROM  dvf_d59.disposition_parcelle_plus t
    LEFT JOIN (SELECT idmutation, iddispopar, idpar FROM dvf_d59.disposition_parcelle_plus WHERE parcvendue = TRUE) tt
    ON t.iddispopar = tt.iddispopar
    GROUP BY t.idmutation                    
...
LEFT JOIN dvf_d62.tmp_calcul_disposition_idmutation t1
	ON t.idmutation = t1.idmutation
LEFT JOIN dvf_d62.tmp_calcul_suf_idmutation t2
	ON t.idmutation = t2.idmutation
LEFT JOIN dvf_d62.tmp_calcul_local_idmutation t3
...
LEFT JOIN dvf_d62.tmp_calcul_disposition_parcelle_idmutation t6
	ON t.idmutation = t6.idmutation
LEFT JOIN dvf_d62.tmp_calcul_annexes_idmutation t7
...
LEFT JOIN dvf_d59.tmp_calcul_volume_idmutation t0
	ON t.idmutation = t0.idmutation
LEFT JOIN dvf_d59.tmp_calcul_disposition_idmutation t1
	ON t.idmutation = t1.idmutation
LEFT JOIN dvf_d59.tmp_calcul_suf_idmutation t2
	ON t.idmutation = t2.idmutation
LEFT JOIN dvf_d59.tmp_calcul_local_idmutation t3
	ON t.idmutation = t3.idmutation
...
LEFT JOIN dvf_d59.tmp_calcul_ann_nature_mutation_idmutation t5
	ON t.idmutation = t5.idmutation
LEFT JOIN dvf_d59.tmp_calcul_disposition_parcelle_idmutation t6
	ON t.idmutation = t6.idmutation
LEFT JOIN dvf_d59.tmp_calcul_annexes_idmutation t7
	ON t.idmutation = t7.idmutation
);
SELECT tablename 
FROM pg_tables 
...
SELECT nextval(pg_get_serial_sequence('dvf.local','iddispoloc')::TEXT);

SELECT nextval(pg_get_serial_sequence('dvf.local_plus','iddispoloc')::TEXT);
SELECT setval(pg_get_serial_sequence('dvf.local_plus','iddispoloc')::TEXT, 118453);
...
ALTER TABLE dvf_d62.local_plus RENAME CONSTRAINT local_plus_pkey TO local_pkeyALTER TABLE dvf_d62.local_plus RENAME CONSTRAINT local_plus_unique TO local_uniqueALTER TABLE dvf_d62.local_plus RENAME CONSTRAINT local_plus_check TO local_checkALTER TABLE dvf_d62.local_plus RENAME To local;

DROP TABLE IF EXISTS dvf_d59.local CASCADE;

ALTER TABLE dvf_d59.local_plus RENAME CONSTRAINT local_plus_pkey TO local_pkeyALTER TABLE dvf_d59.local_plus RENAME CONSTRAINT local_plus_unique TO local_uniqueALTER TABLE dvf_d59.local_plus RENAME CONSTRAINT local_plus_check TO local_checkALTER TABLE dvf_d59.local_plus RENAME To local;
...
SELECT nextval(pg_get_serial_sequence('dvf.disposition_parcelle','iddispopar')::TEXT);

SELECT nextval(pg_get_serial_sequence('dvf.disposition_parcelle_plus','iddispopar')::TEXT);
SELECT setval(pg_get_serial_sequence('dvf.disposition_parcelle_plus','iddispopar')::TEXT, 162126);
...
ALTER TABLE dvf_d62.disposition_parcelle_plus RENAME CONSTRAINT disposition_parcelle_plus_pkey TO disposition_parcelle_pkeyALTER TABLE dvf_d62.disposition_parcelle_plus RENAME CONSTRAINT disposition_parcelle_plus_unique TO disposition_parcelle_uniqueALTER TABLE dvf_d62.disposition_parcelle_plus RENAME CONSTRAINT disposition_parcelle_plus_check TO disposition_parcelle_checkALTER TABLE dvf_d62.disposition_parcelle_plus RENAME To disposition_parcelle;

DROP TABLE IF EXISTS dvf_d59.disposition_parcelle CASCADE;

ALTER TABLE dvf_d59.disposition_parcelle_plus RENAME CONSTRAINT disposition_parcelle_plus_pkey TO disposition_parcelle_pkeyALTER TABLE dvf_d59.disposition_parcelle_plus RENAME CONSTRAINT disposition_parcelle_plus_unique TO disposition_parcelle_uniqueALTER TABLE dvf_d59.disposition_parcelle_plus RENAME CONSTRAINT disposition_parcelle_plus_check TO disposition_parcelle_checkALTER TABLE dvf_d59.disposition_parcelle_plus RENAME To disposition_parcelle;
...
SELECT nextval(pg_get_serial_sequence('dvf.mutation','idmutation')::TEXT);

SELECT nextval(pg_get_serial_sequence('dvf.mutation_plus','idmutation')::TEXT);
SELECT setval(pg_get_serial_sequence('dvf.mutation_plus','idmutation')::TEXT, 106496);
...
ALTER TABLE dvf_d62.mutation_plus RENAME CONSTRAINT mutation_plus_pkey TO mutation_pkeyALTER TABLE dvf_d62.mutation_plus RENAME CONSTRAINT mutation_plus_unique TO mutation_uniqueALTER TABLE dvf_d62.mutation_plus RENAME CONSTRAINT mutation_plus_check TO mutation_checkALTER TABLE dvf_d62.mutation_plus RENAME To mutation;

DROP TABLE IF EXISTS dvf_d59.mutation CASCADE;

ALTER TABLE dvf_d59.mutation_plus RENAME CONSTRAINT mutation_plus_pkey TO mutation_pkeyALTER TABLE dvf_d59.mutation_plus RENAME CONSTRAINT mutation_plus_unique TO mutation_uniqueALTER TABLE dvf_d59.mutation_plus RENAME CONSTRAINT mutation_plus_check TO mutation_checkALTER TABLE dvf_d59.mutation_plus RENAME To mutation;
...
l_codinsee_idx_gin ON dvf_d62.mutation USING GIN (l_codinsee);DROP INDEX IF EXISTS l_codinsee_idx_gin;
CREATE INDEX --IF NOT EXISTS -- a partir de la 9.5
l_codinsee_idx_gin ON dvf_d59.mutation USING GIN (l_codinsee);