...
--
-- Début d'exécution le 24/01/2016 à 18:09:57
...
--FOR EACH ROW EXECUTE PROCEDURE dvf.local_insert_trigger();CREATE TABLE dvf_d59.tmp_calcul_parcelle_iddispopar AS(         
    SELECT
        t.iddispopar, 
        tt.*
    FROM
...
--FOR EACH ROW EXECUTE PROCEDURE dvf.disposition_parcelle_insert_trigger();CREATE TABLE dvf_d59.tmp_calcul_ann_nature_mutation_idmutation AS(         
    SELECT
        t1.idmutation, 
        t2.libnatmut
    FROM
...
LEFT JOIN dvf_d59.tmp_calcul_suf_idmutation t0
...
LEFT JOIN dvf_d62.tmp_calcul_suf_idmutation t0
	ON t.idmutation = t0.idmutation
LEFT JOIN dvf_d62.tmp_calcul_volume_idmutation t1
	ON t.idmutation = t1.idmutation
...
LEFT JOIN dvf_d62.tmp_calcul_local_idmutation t6
	ON t.idmutation = t6.idmutation
LEFT JOIN dvf_d62.tmp_calcul_ann_nature_mutation_idmutation t7
	ON t.idmutation = t7.idmutation
);
